...
// at xx using cubic polynomials.
{
    double deltaX = xx - dx;
    //debug
    double result = (-A0+3.0*A1-3.0*A2+A3)/(6.0*dx*dx*dx)*deltaX*deltaX*deltaX
...
inline double interpCubic4Points(double A0, double A1, double A2, double A3, double dx, double xx, bool limit)
// Assuming that A0, A1, A2, A3 are the values located at 0, dx, 2*dx, and 3*dx, interpolate the value
// at xx using cubic polynomials.
{
    double deltaX = xx - dx;
...
double BinaryRootFinder(double (*func)(double, void* parameters), void*, double value, double xL, double xR, double precision);
int MultiRootFinder(double (*func)(double, void* parameters), void*, double value, double *roots, int division, double xL, double xR, double precision);
...
/*----------------------------------------------------------------------
 ...
 08-02-2012:
 -- Ver 1.8.0:
    All the interpolation methods now support extrapolations.
    The binarySearch function now returns -1 or length-1 for those values out-of-bounds.
    An inline function that does 4 point cubic interpolation / extrapolations added.
 -----------------------------------------------------------------------*/
